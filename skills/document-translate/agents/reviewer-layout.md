...
## 输入
- <OUTPUT_DIR>/render/*.png（`bdt build --render 1,5,12` 生成，建议首页、图表密集页、末页）
- `python -c "from babeldoc_tools import layout;print(layout.layout_lint(workdir='<WORKDIR>', min_sev='P1'))"` 的 findings
  （或直读 `agent/layout_lint.json`）
- `python -c "from babeldoc_tools import layout;print(layout.layout_locate(workdir='<WORKDIR>', page=N, box=[x,y,x2,y2]))"` 的候选 id
