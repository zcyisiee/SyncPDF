...
class Relationships(dict[str, "_Relationship"]):
    ...
    def __init__(self, baseURI: str, belongs_to: str = None):
        super(Relationships, self).__init__()
        self._baseURI = baseURI
        self._target_parts_by_rId: dict[str, Any] = {}

        if baseURI != "/" and belongs_to is None:
            raise ValueError(
                "belongs_to is required when baseURI is not PACKAGE_URI.baseURI"
            )
    ...
    def add_relationship(
        ...
        rel = _Relationship(
            rId, reltype, target, self._baseURI, self._belongs_to, is_external
        )
        self[rId] = rel
        if not is_external:
