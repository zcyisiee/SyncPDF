...
> 进度追踪表见 §7，每完成一格打 `x`。
> **执行修订（2026-09-17，优先于下文旧范围建议）**：用户已确认本轮完成 M0–M6 全部能力后统一验收；M3 只是中间演示。成功编译的工作版允许下载，明确显示质量状态与修订号；失败编译保留上一版并提示其不含最新修改。详见 `EXECUTION.md`。
...
- **事件持久化不等于任务恢复**：必须持久化 job 状态和进程身份，服务重启核对任务并恢复观察或标记 interrupted；不可仅凭 PID 杀进程、自动重跑收费调用或永久锁死文档。SSE 游标必须包含 run 身份。
...
**本轮确认覆盖 M0–M6 全部能力**：以上分版仅保留为原估算参考，不作为停止条件。页级重排沿用已决定的全量回退，不因增量编译阻塞交付。最终由用户通过真实浏览器完成上传、翻译、修改、局部重译、编译及下载；bbox 拖拽、版本归档/回滚、词表与逐段进度也纳入验收。
...
- [x] 本轮范围：M0–M6 全部能力统一验收（2026-09-17 用户问卷）
- [x] 下载：允许最近成功编译产物，明确质量状态与修订号；归档门禁保留
