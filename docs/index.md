...
| 工作台设计稿 | 面向普通用户的界面原型：`web/design/README.md` 与 `web/design/index.html`（不在本站内，故不加站内链接） | 调整工作台版式与交互，或把它接入 `web/src` 之前 |
...
- 工作台设计稿是上述「不新增设计稿」原则的**一个临时例外**：`web/design/index.html` 是本轮工作台改造的定稿依据，跟着代码走版本控制；对应能力接入 `web/src` 后删除该目录，不把它当长期资产。查看方式见 [运行与验证](guide/cli.md)。
