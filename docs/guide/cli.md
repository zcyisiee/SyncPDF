...
项目 Python 版本约束在 `pyproject.toml`；本地验证使用仓库 `.venv`，当前环境为 Python 3.12。
...
mkdir -p tmp/library
bdt serve --root tmp/library --port 8787
...
```bash
ps eww "$(pgrep -f 'bdt serve --root')" | tr ' ' '\n' | grep -c MINERU_API_TOKEN   # 0 = 没带上
```
...
不要把下面的本地 pytest 与真实模型翻译验收混为一谈。功能改动运行相关测试；完整回归必须让测试找到仓库 `.venv/bin/bdt`：

```bash
export PATH="$PWD/.venv/bin:$PATH"
.venv/bin/python -m pytest --basetemp="tmp/pytest-$(date +%Y%m%d-%H%M%S)"
# 仅入口守卫：
.venv/bin/python -m pytest tests/test_single_entry.py \
  --basetemp="tmp/pytest-entry-$(date +%Y%m%d-%H%M%S)"
# 对改动的 Python 文件运行；不要向 Ruff 传 Markdown 文件
.venv/bin/ruff check <files>
.venv/bin/mkdocs build --strict --site-dir "tmp/docs-$(date +%Y%m%d-%H%M%S)"
git diff --check
```
