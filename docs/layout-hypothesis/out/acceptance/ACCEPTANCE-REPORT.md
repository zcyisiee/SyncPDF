...
`line-fill-improvement.json`：applied 段落的非末行最小填充率对比——
2026-f1872 5/5、LawBench 1/1 全部改善（avg 0.823→1.0、0.844→1.0）；
DeepSeek 21/28 改善，2 段的指标回退经核实为**下标伪行测量伪影**
（`𝑙` 下标在文本抽取中形成独立伪行，default 同样存在；LaTeX 真实文本行
fill=1.000 vs default 0.945，实际为改善）。

新发现的化妆级限制：MinerU 公式源码尾部句点与译文中文句号可叠出
`Hl.。`（公式源标点归一化留待后续）。
