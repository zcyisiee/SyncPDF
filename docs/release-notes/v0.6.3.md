...
This release includes a CMap loader fix reported via coordinated disclosure,
together with one additional defense-in-depth hardening shipped alongside it
(the ImageWriter path sanitization listed above). The advisory identifier
and link will be added here after publication.

本版本包含一处通过协调披露收到的 CMap 加载器安全修复，并同时附带上文 Highlights
里列出的一项纵深防御加固（ImageWriter 路径清理）。Advisory 标识与链接将在公开
后追加。
