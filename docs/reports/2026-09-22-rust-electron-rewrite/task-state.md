...
> 状态：**绑定候选及新增stale、CID/GID、trailer Size修复已合入并复验。全篇质量仍不通过，R1整批未标通过，未启动R2；整体后端任务未完成。**
...
- 每批整体验收通过才启动下一批；同一 worktree 一个 writer。恢复 dirty 前确认旧任务停止并保存证据，不从头覆盖已有工作；当前绑定恢复任务不提交，由主控最终统合。
...
- **唯一下一步**：按原门限针对R1剩余布局/定位做单项返工：p7/p15 coverage gap及跨运行Tm基线约0.05–0.11pt漂移。尚无新worker；不得重新调查p19或重启旧任务。R1 gate未过不拆R2；源文/事务/状态及排版质量仍按后续顺序处理。
...
- Orca 树 `../codex-r1-review` 的只读review与stale窄修worker均已结束，无writer；337行精确delta已由主控审阅并统合。字体树worker也已结束；没有活动叶子任务。

## 5. 恢复入口与证据（详情不在本文件复制）

- 主树：`/Users/zhengcaiyi/orca/workspaces/ieeTranslater/桌面端`；绑定树：同级 `repair-r1-bind`，分支 `zcyisiee/repair-r1-bind`，原基线 `e925d3b6`，继承 dirty 全部保全并只提交10文件为 `b47c7283`；主树文档dirty与无关cache仍保留。brief 必须给出本文件在主树的绝对路径，叶子跨树只读。
