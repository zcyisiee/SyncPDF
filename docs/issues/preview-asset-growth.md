...
- 状态：**部分修复，验收未完成**。当前流式路径已拆分块 patch、页面发布和结束阶段整本合成；真实文档性能、质量门禁和历史资产回收尚未完成。
- 证据来源：本机 `tmp/app.db`、`tmp/up-alns-20260919-092753/`、`tmp/up-trc-20260919-054344/` 的真实运行数据，以及 `pymupdf` 复刻实验

## 历史缺陷与当前修复边界

以下运行数字和旧代码片段记录修复前行为，不代表当前流式实现。当前 `compile_block_patch()` 只保存贴片，`compose_page_asset()` 发布页资产，`compose_full_preview()` 从 baseline 与页资产合成完整 PDF。历史资产未删除，只有完成引用扫描与 dry-run 后才可清理，不能按 `kind` 直接删除。
...
该小型复刻证明孤儿对象会增长，但不能推断真实 PDF 的增长全部来自孤儿对象。真实文档从上一版整本 PDF 反复导入页面资源时，还会产生新的可达资源；垃圾回收不能删除仍被引用的对象。根治路径是流式期间只维护从 immutable baseline 合成的页资产，结束时再合成整本 PDF，而不是仅给每个块的整本保存增加 garbage 参数。
...
历史资产回收仍未实施。必须扫描全部有效引用并输出 dry-run 清单，再决定清理；不得按 `preview` kind 直接删除。
