...
部分段落回退、源区域归属冲突或 coverage gap 时，保存可读的部分结果并发 `translation_incomplete`，RunFinished/RunSummary 的 ok=false，CLI 非零退出。reference/脚注等按策略保留的内容不算失败。普通数字原子以准确原文回填，可跨相邻源样式；在目标样式片段内跟随模型选定的已知样式，无目标样式时继承第一个源字形样式。`Other` 中严格匹配 `[2]` / `[17, 8]` 的数字引用也可回填，但额外要求阅读序span完整、连续覆盖源glyph范围且文本精确一致；不更改Markdown传输版本或缓存键，KEEP身份仍须通过校验。HTTP(S) URL也要求完整精确源span后按原文恢复。链接准备统一覆盖有atom和无atom段：KEEP内引用使用源偏移和原子身份；旧缓存未标记引用要求唯一文本匹配，重复标签只能在目的地一致或中英文表/节等上下文足以消歧时接受。内部临时样式标记携带锚点到目标字形，按行生成Rect/QuadPoints，在页候选事务中更新原注释且保留Dest/A。模糊匹配、非Link注释或跨段注释所有权不清仍拒绝，不能简单删除注释门禁。CCS实测319链接保留、233个框移动、全部点击标签对应检查通过，原22错位消除；这不是任意语言/任意注释格式的认证。公式及不支持的Other仍保护回退。

段落空格/阅读序、逐run样式、完整公式源绘制及超出上述范围的链接仍需各自验收。`links_check()`本身仍只检查数量/目标解析；不能单独作为点击几何验收，须结合锚点与保存后PDF检查。生产 Rust 排版不调用 LaTeX；TeX/旧 bdt 可用于质量对照。
